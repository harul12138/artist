...
<?php
	...
	if(is_array($_POST)&&count($_POST)>0)//先判断是否通过Post提交了请求
    {
	        ...
	        if(isset($_POST["id"]))
	       {
	        ...
	        {
				$typical=strip_tags($_POST['typical'] );
			}
	   ...
	   if(!empty($_POST["category"])){
	      mysql_query("set names 'utf8'");
			$sql = "UPDATE `artwork` SET `title` = '$title',`category` = '$category',`typical` = '$typical',`prize` = '$prize',
				`info` = '$info' WHERE `artwork`.`id` =$id";
			 $result=mysql_query($sql);  
         if(!$result)
        {
  
          echo"<script type='text/javascript'>alert('作品信息修改失败');; 
      location='artwork.php';</script>"; 
        }
        else
        {
      ...
      location='artwork.php';</script>"; 
         ...
         mysql_query("set names 'utf8'");
			$sql = "UPDATE `artwork` SET `title` = '$title',`typical` = '$typical',`prize` = '$prize',
				`info` = '$info' WHERE `artwork`.`id` =$id";
			 $result=mysql_query($sql);  
         if(!$result)
        {
	...
	if(is_array($_GET)&&count($_GET)>0)//先判断是否通过get提交了请求
    {
        if(isset($_GET["id"]))
        {
		...
		mysql_query("set names 'utf8'");
	     $sql="select * from artwork where id=".$id;//查询指定文章的详细内容
	      $sq=mysql_query($sql);
				...
				<div class="main-content">
					<div class="breadcrumbs" id="breadcrumbs">
						...
						<ul class="breadcrumb">
							...
							<li>
								<a href="artwork.php">查看作品列表</a>
							</li>
							<li class="active">作品内容编辑</li>
						</ul><!-- .breadcrumb -->
					...
					<div class="page-content">
						<div class="row">
							<div class="col-xs-12">
								<!-- PAGE CONTENT BEGINS -->
                                    <form class="form-horizontal" role="form" action="editwork.php" method="post">
									...
									<option value="<?php echo $list["category"];?>" selected="selected"><?php echo $list["category"];?></option>
													<?php
											      $ca=$list["category"];		
                                      require_once ("mysql_connect.php");
                                       mysql_query("set names 'utf8'");
                                    ...
                                    <span class="help-button" data-rel="popover" data-trigger="hover" data-placement="left" 
                                     ...
                                     <div class="col-sm-9">
										...
										<div class="radio">
													<label>
														<input name="typical" type="radio" class="ace" value="yes" <?php if($list["typical"] != "no") echo "checked=checked;"?>/>
												...
												<div class="radio">
													<label>
														<input name="typical" type="radio" class="ace" value="no" <?php if($list["typical"] == "no") echo "checked=checked;"?>/>
									...
									<div class="form-group">
										...
										<div class="col-sm-9">
											<input data-rel="tooltip" type="text" name="prize" id="form-field-6" value="<?php echo $list["prize"];?>" title="若上一项为否此项可空" data-placement="bottom" class="col-xs-10 col-sm-5" />
