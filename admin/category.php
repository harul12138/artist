 ...
 require_once ("mysql_connect.php");
  if(is_array($_REQUEST)&&count($_REQUEST)>0)//先判断是否通过Post提交了请求
    ...
    if(!strcmp($reqAct,"delete"))
    {
      ...
      if(intval($id)>=0)
      {
        $sql = "delete from artfam where id=" .$id;
        $result=mysql_query($sql);  
         if(!$result)
        {
          
          echo"<script type='text/javascript'>window.alert('类别删除失败');; 
      location='category.php';</script>"; 
        }
        else
        {
          echo"<script type='text/javascript'>window.alert('类别删除成功');; 
      location='category.php';</script>"; 
        }
      }
    }
...
?>
		...
		<div class="main-container" id="main-container">
			...
			<div class="main-container-inner">
							...
							<div class="col-xs-12">
								<!-- PAGE CONTENT BEGINS -->
								 <div class="alert alert-warning">
											...
											由于作品展示页面是按照分类的id读取的，请尽量在原有类别上进行修改，不要进行删除操作。
											<br />
										</div>
                                  	<div class="row">
									<div class="col-xs-12">
             ...
             mysql_query("set names 'utf8'");
              $cc="select * from artfam order by id"; 
    ...
    {   ?>   
														...
														<td>
															<?php echo $ca["category"];?>
														</td>
														<td><?php echo $ca["abstr"];?></td>
														<td><?php echo $ca["info"];?></td>
														<td>
															<div class="visible-md visible-lg hidden-sm hidden-xs btn-group">
																
																<a class="btn btn-xs btn-info" href="modicateg.php?action=modify&id=<?php echo $ca["id"];?>" title="Edit" >
																	<i class="icon-edit bigger-120"></i>
																</a>

																<a class="btn btn-xs btn-danger" href="category.php?action=delete&amp;id=<?php echo $ca["id"];?>" title="Delete">
