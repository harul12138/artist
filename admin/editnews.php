...
<?php
	...
	if(is_array($_POST)&&count($_POST)>0)//先判断是否通过Post提交了请求
    {
	      ...
	      if(isset($_POST["id"]))
	       {
				$id=strip_tags($_POST['id'] ); //strip_tags() 函数剥去字符串中的 HTML、XML 以及 PHP 的标签。
			}
	    ...
	    mysql_query("set names 'utf8'");
			$sql ="UPDATE `news` SET `date` = '$date',`title` = '$title',`website` = '$website',`content` = '$content'
			WHERE `news`.`id` =$id";
			 $result=mysql_query($sql);  
         if(!$result)
        {
  
          echo"<script type='text/javascript'>alert('新闻编辑失败');; 
      location='editnews.php';</script>"; 
        }
        else
        {     
          echo"<script type='text/javascript'>alert('新闻编辑成功');; 
	...
	if(is_array($_GET)&&count($_GET)>0)//先判断是否通过Post提交了请求
    {
		...
		mysql_query("set names 'utf8'");
	     $sql="select * from news where id=".$id;//查询指定新闻的详细内容
	      $sq=mysql_query($sql);
				...
				<div class="main-content">
					...
					<div class="page-content">
						<div class="row">
							<div class="col-xs-12">
								<!-- PAGE CONTENT BEGINS -->
                                    <form class="form-horizontal" role="form" action="editnews.php" method="post" enctype="multipart/form-data">
                                 <input type="hidden" name="id" value="<?php echo $nn["id"]; ?>">
									<div class="form-group">
										<label class="col-sm-3 control-label no-padding-right" for="form-field-1"> 新闻标题 </label>

										<div class="col-sm-9">
											<input name="title" type="text" id="form-field-1" class="col-xs-10 col-sm-5" value="<?php echo $nn["title"];?>" />
										</div>
									</div>
                                    ...
                                    <div class="form-group">
										<label class="col-sm-3 control-label no-padding-right" for="form-field-1"> 相关链接</label>

										<div class="col-sm-9">
											<input name="website" type="text" id="form-field-1" class="col-xs-10 col-sm-5" value="<?php echo $nn["writer"];?>" />
										</div>
									</div>
														...
														<div class="row">
															<div class="col-xs-6 col-sm-3">
																<div class="input-group">
																	...
																	type="text" data-date-format="yyyy-mm-dd" value="<?php echo $nn["date"];?>"/>
									...
									<div class="space-8"></div>
										...
										<div class="col-sm-9">
											<input readonly="" name="category" type="text" class="col-xs-10 col-sm-5" 
											id="form-input-readonly" value="个人新闻" />
                                 ...
                                 <div class="space-4"></div>
									...
									<div class="form-group">
										...
										<div class="col-sm-6">
											<textarea class="form-control editable" name="content" id="about" rows="8"><?php echo $nn["content"];?></textarea>
										</div>
									</div>
									<div class="space-4"></div>
