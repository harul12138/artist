...
<?php
	...
	if(is_array($_POST)&&count($_POST)>0)//先判断是否通过Post提交了请求
    {
	      ...
	      mysql_query("set names 'utf8'");
			$sql ="UPDATE `timeline` SET `date` = '$date',`event` = '$event' WHERE `timeline`.`id` =$id";
			 $result=mysql_query($sql);  
         if(!$result)
        {
  
          echo"<script type='text/javascript'>alert('事件编辑失败');; 
      location='moditime.php';</script>"; 
        }
        else
        {     
          echo"<script type='text/javascript'>alert('事件编辑成功');; 
      location='timelist.php';</script>"; 
        }
	...
	if(is_array($_GET)&&count($_GET)>0)//先判断是否通过Post提交了请求
    {
		...
		mysql_query("set names 'utf8'");
	     $sql="select * from timeline where id=".$id;//查询指定文章的详细内容
	      $sq=mysql_query($sql);
				...
				<div class="main-content">
					<div class="breadcrumbs" id="breadcrumbs">
						...
						<ul class="breadcrumb">
							...
							<li>
								<a href="timelist.php">时间轴管理</a>
							</li>
							<li class="active">时间轴编辑</li>
						</ul><!-- .breadcrumb -->
					...
					<div class="page-content">
						<div class="row">
							<div class="col-xs-12">
								...
								<form action="moditime.php" method="post" class="form-horizontal" role="form">
								<input type="hidden" name="id" value="<?php echo $tt["id"]; ?>">
	                               <label class="col-sm-3 control-label no-padding-right" for="id-date-picker-1" >选择日期</label>
														...
														<div class="row">
															<div class="col-xs-6 col-sm-3">
																<div class="input-group">
																	<input name="date" class="form-control date-picker" id="id-date-picker-1"
																	 type="text" data-date-format="dd-mm-yyyy" value="<?php echo $tt["date"];?>"/>
										...
										<label class="col-sm-3 control-label no-padding-right" for="form-field-1"> 事件 </label>

										<div class="col-sm-6">
											<textarea name="event" class="form-control" rows="7"><?php echo $tt["event"];?></textarea>
										</div>
									</div>
